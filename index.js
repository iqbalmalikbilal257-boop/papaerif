...
function loadBoardData(board) {
  try {
    ...
    if (safeBoard === 'punjab') {
      ...
      const punjabFiles = fs.readdirSync(syllabusDir)
        .filter(f => f.startsWith('punjab_class') && f.endsWith('.json'))
        .sort();
      
      const allData = {};
      punjabFiles.forEach(file => {
        const filePath = path.join(syllabusDir, file);
        const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        data.forEach(classData => {
          const cls = classData.class;
          if (!allData[cls]) allData[cls] = { class: cls, subjects: [] };
          allData[cls].subjects.push(...classData.subjects);
        });
      });
      return Object.values(allData).sort((a, b) => parseInt(a.class) - parseInt(b.class));
    }
